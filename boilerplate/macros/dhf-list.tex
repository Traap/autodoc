...
% ------------------------------------------------------------------------------
% Define column names used by csvreader for the \Gls{dhf}.
\csvnames{dhfNames}%
{1=\pj,2=\pv,3=\ty,4=\dn,5=\ver,6=\ttl,7=\pkg,8=\dra,9=\drb,10=\drc,11=\drd
,12=\dre,13=\ai}%

% Define a table style for the \Gls{dhf} csvreader.
\csvstyle{dhfStyle}{%
...
% ------------------------------------------------------------------------------
% Define commands to select rows from the \Gls{dhf}.
% Parameters:
% #1 - relative path to \Gls{dhf}.
...
% -----------------------------------------------------------------------------
% Define a command to query rows from the \Gls{dhf}.
% Parameters:
% #1 - relative path to \Gls{dhf}.
%
...
% \Gls{dhf}.
% Parameters:
% #1 - relative path to the reference documents \Gls{dhf}.
% #2 - title value
...
% ------------------------------------------------------------------------------
% Define a command to select a row from the Product \Gls{dhf}.
% Parameters:
% #1 - relative path to the Product \Gls{dhf}.
% #2 - title value
\newcommand{\dhfProductDocList}[2]{\dhfProductDocQuery{#1}{\csvcolvi}{#2}}%

% -----------------------------------------------------------------------------
% Define a command to query a row from the Product \Gls{dhf}.
% Parameters:
% #1 - relative path to a \Gls{dhf}.
% #2 - first column name to match (\ttl)
% #3 - first column value to match
\newcommand{\dhfProductDocQuery}[3]{%
  \IfFileExists{#1}{%
      \csvreader[separator=pipe, respect all,%
      filter={\equal{#2}{#3}}]%
      {#1}{}{{\csvcoliv}[{\csvcolv}]}%
