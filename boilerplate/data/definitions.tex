% {{{ docTerm 
% 
% \docTerm uses three paramaters to define a glossary entry: 
%   #1 - key
%   #2 - name
%   #3 - description
%
...
%
%
% \docTermPlural adds a fourth add a fourth parameter:
%   #4 - plural form of #2.
% ------------------------------------------------------------------------------
\newcommand{\docTerm}[3]{%
  \newglossaryentry{#1}{%
...
% {{{ [0..0]
\docTerm{510k}{510(k)}{
  A premarket submission made to FDA to demonstrate that a medical device to be
...
\docTerm{abbr510k}{abbreviated 510(k)}{
  ...
  reduces the amount of information supplied to the FDA in the pre-market
  notification and facilitates FDA review utilizing:
...
\docTerm{acceptenace-criteria}{acceptenace criteria}{
  1) Specified set of operating pereameter values that, if deviated from,
...
\docTermPlural{accessory}{accessory}{
  ...
  accordance with the manufacturer's intended use of the devic.
}{accessories}
...
\docTerm{adverse-event}{adverse event}{
  ...
  a patient. The event is serious and should be reported to FDA when the patient
...
\docTerm{annual-report}{annual report}{
  Report submitted annually to the FDA by the applicant of a New Drug
...
\docTerm{awaredate}{aware date}{
  The date Fenwal has determined a complaint to meet requirements for reporting
  ...
  2) Additional information is acquired that results in a decision to change
     a prior determination of Non-reportable to Reportabl.
}
...
\docTerm{beta-evaluation}{beta evaluation}{
  ...
  allowed by \gls{company} processe.
}
...
\docTerm{clinical-evaluations}{clinical evaluations}{
  ...
  of the device/drug including evaluations for regulatory submissio.
}
...
\docTerm{clinical-protocol}{clinical protocol}{
  ...
  2) Provide brief background information, instructions, and details of the
     steps to be followed in conducting the stud.
}
...
\docTerm{configuration-mangement}{configuration mangement}{
  (IEEE)
...
\docTerm{contentre-engineering}{content re-engineering}{
...
\docTerm{contract-product-developer-or-manufacturer}{contract product developer or manufacturer}{
...
\docTerm{corrective-action}{corrective action}{
  ...
  product, process, or quality system nonconformance, defect, trend  or or other
...
\docTerm{design-dossier}{design dossier}{
  ...
  assessment when applying for CE Mar.
}
...
\docTerm{deviation}{deviation}{
  A detected or potential nonconformance, system deficiency, or non-fulfillment
...
\docTerm{effectiveness-check}{effectiveness check / verification}{
  ...
  adressed the identified nonconformity.
}
...
\docTerm{excipient}{excipient}{
  Those components of a finished medicinal drug product other than the API. They
  are included in the formulation to facilitate manufacture, enhance stability,
  control release of API from the product, assist in product identification or
  enhance other potential characteristics.
}
...
\docTerm{harmonized-standard}{harmonized standard}{
  ...
  following a request from the European Commission to one of these organisation.
}
...
\docTerm{inspection}{inspection}{
  ...
  external audit.  Inspections are performed by FDA, Notified Bodies, PMDA, TGA,
  etc.
}

\docTerm{installer}{installer}{
  Installer is a software-only utility that is composed of VSS Installer,
  Sentinel Installer, and Device Uploader Installer.  It is located on the VSS
  distribution media.  Installer is designed to assist a qualified systems
  administrator to install, uninstall, and configure the VSS subsytems.
  1) VSS Installer manages Vigilant Bridge, Vigilant Centerium, Vigilant
     Insight, and Vigilant Master Med;
  2) Sentinel Installer manages Vigilant Sentinel; and
  3) Device Uploader Installer manages Device uploader.  Enabling and disabling
     of VSS features and functions are controlled by the feature license
     function not by Installer.
}
...
\docTerm{internal-requirements}{internal requirements}{
  Requirements that originate within the ABT-BU organizational entity.  Internal
...
\docTerm{investigation}{investigation}{
  ...
  a nonconformance in order to determine Root Caus.
}
...
\docTerm{laboratory-notebook}{laboratory notebook}{
  A bound, prenumbered book with numbered pages and an index (or Table of
...
\docTerm{latex}{\LaTeX}{
  is a high-quality typesetting system, it includes features designed fo rthe
  production of technical and scientific documents, and documents requireing
  percice typesetting requirements.
}

\docTerm{legal-manufacturer}{legal manufacturer}{
  Entity responsible for the design, manufacture, packaging and labelling of
  a device before it is placed on the market under..
}
...
\docTerm{market-withdrawal}{market withdrawal}{
  ...
  violation of the act that would not be subject to legal action by FDA or that
...
\docTerm{objective-evidence}{objective evidence}{
  ...
  pertaining to: 1) The quality of an item or service; 2) The existence and
  implementation of a quality system element, which is based on observation,
...
\docTerm{oem-product}{oem product}{
  Product(s) (finished devices, drug or biologics) used and/or sold by Fresenius
  Kabi, which are manufactured by an OEM Supplier. This includes products that
...
\docTerm{organizational-entity}{organizational entity}{
  ...
  in a particular way with another to perform and manage business.  Fresenius
  Kabi organizational entities include Corporate, Group, Business Unit and Site
  entities.
}
...
\docTerm{performance-spec}{performance specificagtion}{
...
\docTerm{preventive-action}{preventive action}{
  ...
  undesirable situation in order to prevent occurence.
}
...
\docTerm{product}{product}{
  ...
  use, and any item subject to a quarantine regulation of the FDA.
}
...
\docTerm{productin-field}{product in field}{
...
\docTerm{qms-document}{qms document}{
  ...
  to the distinct Document Categories that make up the QMS. QMS documents can be
  paper or electronic, and are categorized as policies, standard operating
...
\docTerm{quality-manual}{quality manual}{
  A QMS Document (Policy) that defines the ABT-BU organizational entity,
  establishes QMS scope and structure, and outlines management oversight
  responsibilities.
}
...
\docTerm{re-audit}{re-audit}{
  An audit that occurs in response to previously identified Nonconformances for
...
\docTerm{regulatory-opinion}{regulatoryopinion}{
...
\docTerm{remediationplan}{remediation plan}{
  ...
  or potential nonconformance/exception or other undesirable situation to
...
\docTerm{report}{report}{
  A display of data / information used for creating awareness and / or decsion
  making.
}
...
\docTerm{risk-control}{risk control}{
  ...
  2) Specific requirements that are implemented to reduct the severity of the
...
\docTerm{risk-managementfile}{risk management file}{
...
\docTerm{root-cause-analysis}{root cause analysis}{
  ...
  a system, product or process nonconformance; this effort extends beyond the
  effects of a problem to discover its most fundamental caus.
}
...
\docTerm{ruby}{ruby}{
  ...
  programming paradigims, including procedueral, object-oriented, and functional
  programming.
}

\docTerm{rubygem}{RubyGem}{
  (WikipediA) A package manager for the \gls{ruby} programming language that
  provides a standard format for distributing \gls{ruby} programs and libaries
...
\docTerm{site}{site}{
  ...
  unique FDA registrations and/or notified body certifications.
}
...
\docTerm{sub-tiersupplier}{sub-tier supplier}{
...
\docTerm{supply-risk}{supply risk}{
  The actual or potential business impact that a supplier poses to Fresenius
  Kabi.
}
...
\docTerm{trend-analysis}{trend analysis}{
  ...
  permanent improvement of the proces.
}
...
\docTerm{validation}{validation}{
  ...
  that the particular requirements for a specific intended can be consistently
  fulfilled.
