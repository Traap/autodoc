...
This Test Plan describes Test Suites, Test Cases, and Test Steps that demonstrate
...
IURs and the steps that demonstrate how the Configuration Item
satisfies the IUR\@.  Each Test Step is marked passed or failed as it is
completed.  Each Test Plan is marked as passed when all Test Steps pass or as failed
...
\begin{description}
  \item[\LaTeX] files are used to provide an Abstract, Introduction, Intended
    ...
    and assembled into PDF documents.  PDF documents are routed using the Company's
    document management system for approval.

  \item[Ruby] software is used to run the automated framework to collect test
    evidence.

  \item[Git] is used as the storage repository for \LaTeX\ \& YAML files, and a Git
    pull request is used to review the \LaTeX\ \& YAML files prior to use.

  \item[Evidence] Test Plan output includes one Test Suite, one Test Plan, one Test
