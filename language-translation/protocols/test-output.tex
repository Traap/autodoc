% Define column names use by csvreader for Test Case results. 
\csvnames{tcNames}{1=\id, 2=\key, 3=\des, 4=\status}

% Define a table style for Test Case results. 
\csvstyle{tcStyle}{
  longtable=|L{1.5cm}|L{5cm}|L{7cm}|C{1.5cm}|
  ,table head=\hline REQID & RESKEY & TEXT & STATUS\\\hline\hline\endhead
  ,late after line=\\\hline
  ,tcNames
}

% Define a command to standardize test case output (tco).
\newcommand{\tco}[1]{
  %
  \global\edef\tcPath{test-output/en/factory/case}
  \global\edef\tcLog{\tcPath/#1/#1.log}
  \global\edef\tcPng{\tcPath/#1/#1.png}
  \global\edef\tcCsv{\tcPath/#1/#1.csv}
  %`
  \clearpage
  \subsection{#1 Test Case}
  %
  \lstinputlisting{\tcLog}
  %
  \begin{figure}[ht]
    \centering
    \includegraphics[scale=0.25]{\tcPng}
    \caption{#1 Page}
  \end{figure}
  %
  \csvreader[tcStyle, separator=pipe]
  {\tcCsv}{}{\id & \key & \des & \status}
}

% ------------------------------------------------------------------------------

\clearpage
\section{English}
\subsection{Test Plan}
\lstinputlisting{test-output/en/factory/plan/english/english.log}

% ------------------------------------------------------------------------------

\clearpage
\subsection{Test Suite}
\lstinputlisting{test-output/en/factory/suite/english/english.log}

% ------------------------------------------------------------------------------
\tco{about}
\tco{configuration}
\tco{data-sets}
\tco{devices}
\tco{home}
\tco{hospital}
\tco{languages}
\tco{logout}
\tco{organization}
\tco{roles}
\tco{select-language}
\tco{vir}

% ------------------------------------------------------------------------------
